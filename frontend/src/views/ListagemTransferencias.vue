<!-- 
  ListagemTransferencias.vue - Componente de visualização da listagem
  Responsável por:
  - Exibir a lista de transferências agendadas
  - Gerenciar o estado da página
-->
<template>
  <div class="container">
    ...
    <table class="transferencias-table">
      ...
      <tbody>
        <tr v-for="t in transferenciasOrdenadas" :key="t.id">
          ...
          <td>R$ {{ formatarValor(t.taxa) }}</td>
          <td>R$ {{ formatarValor(parseFloat(t.valor) + parseFloat(t.taxa)) }}</td>
